...
class SyntheticDailyBarWriter(BcolzDailyBarWriter):
    ...
    def _raw_data_for_asset(self, asset_id, dates):
        """
        Generate 'raw' data that encodes information about the asset.

        See class docstring for a description of the data format.
        """
        assert dates[0] > self.PSEUDO_EPOCH

        # Truncate off dates for which this asset didn't exist according to our
        # asset info.
        dates = dates[
            dates.slice_indexer(
    ...
    def gen_ctables(self, dates, assets):
        for asset in assets:
            yield asset, self._raw_data_for_asset(asset, dates)
